...
- Opening15 expanded path now carries a **hashed eligibility/evidence manifest** (`eligibility.json` + `decision.eligibility`) from `cycle.select` through monitor / evaluate / resolve. The comparison universe is declared before inference. A hygiene-shortlist-only contract with a valid context cite evaluates or lands `observation_inadequate` (explicit missing-data) instead of a packet-only "unknown contract/evidence" reject. Selector_v2 (active) and selector_v3 (shadow only) prompts name that declared universe. Tuesday `research.cli run` / `recommend()` stay packet-printed; `evaluate_core` keys and packet-only P&L marks are unchanged.
- Opening15 evaluation reports emit **arm identity** (`experiment_id`, model, backend, prompt/version, universe, hygiene/cost config). The five-session protocol **rejects missing or mixed identities** so Tue packet-baseline cannot pool with expanded select. Same-K selection quality is separate from enter/abstain-vs-always-flat; predeclared fixed-K=1/2/3 arms keep abstain days scoreable. Observed `not_filled` stays the Tuesday no-fill label; expanded-only `observation_inadequate` is not scored as a successful flat. Incomplete random/fixed-K draws are reported.
